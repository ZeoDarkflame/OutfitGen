...
class CompatModel(nn.Module):
    def __init__(
        ...
        self.cnn = cnn
        self.need_rep = need_rep    # whether or not to output the prior layer of network
        self.num_rela = 15 * len(conv_feats)
        self.bn = nn.BatchNorm1d(self.num_rela)  # this will have 5x5=25 elements
    ...
    def _compute_type_repr_loss(self, tmasks, features):
        ...
        # Type embedding loss
        tmasks_loss = tmasks.norm(1) / len(tmasks)      #makes the mask sparse
        features_loss = features.norm(2) / np.sqrt(     #regularizes to make the vector a unit vector
    ...
    def _compute_score(self, images, activate=True):
        ...
        if batch_size == 1:
            ...
            relations = torch.stack(relations, dim=1) # (32 ,15x4=60)
